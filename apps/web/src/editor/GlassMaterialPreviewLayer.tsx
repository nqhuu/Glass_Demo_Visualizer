            ...
            return (
              <g key={`defs-${id}`}>
                ...
                <linearGradient id={`glass-highlight-${id}`} x1="0%" x2="100%" y1="0%" y2="100%">
                  <stop offset="0%" stopColor="#ffffff" stopOpacity={getHighlightOpacity(product)} />
                  <stop offset="42%" stopColor="#ffffff" stopOpacity="0.04" />
                  <stop offset="100%" stopColor={product.baseColor} stopOpacity={getBaseOpacity(product) * 0.55} />
                </linearGradient>
                <linearGradient id={`glass-reflection-${id}`} x1="0%" x2="100%" y1="100%" y2="0%">
                  <stop offset="0%" stopColor="#ffffff" stopOpacity="0" />
                  <stop offset="44%" stopColor="#ffffff" stopOpacity={getReflectionOpacity(product)} />
        ...
        region.panes.map((pane) => {
          ...
          return (
            <g key={`glass-preview-${id}`} clipPath={`url(#glass-clip-${id})`}>
              <polygon points={pointsToSvg(pane.panePointsJson)} fill={product.baseColor} opacity={getBaseOpacity(product)} />
...
function getBaseOpacity(product: GlassProduct): number {
  ...
  return clamp(0.05 + product.tintStrength * 0.22 + (1 - product.transmissionLevel) * 0.18 + typeBoost, 0.08, 0.48);
}

function getHighlightOpacity(product: GlassProduct): number {
  return clamp(0.1 + product.transmissionLevel * 0.18 + product.reflectivityLevel * 0.18, 0.12, 0.42);
}

function getReflectionOpacity(product: GlassProduct): number {
  const boost = product.materialType === 'reflective' ? 0.22 : 0;
  return clamp(0.06 + product.reflectivityLevel * 0.28 + boost, 0.08, 0.5);
}
